  ...
  return (
    <DashboardLayout>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {tutors.map((tutor) => (
            <div key={tutor.id} className="glass-panel rounded-2xl p-6 flex flex-col hover:border-accent/30 transition-all duration-300">
              ...
              <div className="flex items-center justify-between mt-auto pt-4 border-t border-white/10">
                ...
                <button 
                  ...
                  className="px-4 py-2 bg-white/10 hover:bg-accent hover:text-background text-white rounded-xl text-sm font-bold transition-all"
                >
                  Book Session
                </button>
              </div>
...
function BookingModal({ tutorId, onClose }: { tutorId: number, onClose: () => void }) {
  ...
  const bookMutation = useCreateSession({
    mutation: {
      ...
      onError: () => toast({ variant: "destructive", title: "Error", description: "Could not book session." })
    }
  });
      ...
      <div className="glass-panel w-full max-w-md rounded-3xl p-6 relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input required value={subject} onChange={e => setSubject(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 text-white focus:border-accent" placeholder="e.g. Calculus 101" />
          </div>
          
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-white/80 mb-2">Date</label>
              <input type="date" required value={date} onChange={e => setDate(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 text-white focus:border-accent" />
            </div>
            <div>
              <label className="block text-sm font-medium text-white/80 mb-2">Time</label>
              <input type="time" required value={time} onChange={e => setTime(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 text-white focus:border-accent" />
            </div>
          </div>
          
          <div>
            <label className="block text-sm font-medium text-white/80 mb-2">Notes for Tutor (Optional)</label>
            <textarea value={notes} onChange={e => setNotes(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 text-white focus:border-accent min-h-24" placeholder="What do you need help with?" />
          </div>
          ...
          <div className="flex gap-3 mt-6">
            ...
            <button type="submit" disabled={bookMutation.isPending} className="flex-1 py-3 rounded-xl bg-accent text-background font-bold hover:opacity-90 flex justify-center items-center">
