...
// Vercel's serverless functions don't have persistent local disk storage —
// files written to disk (even /tmp) during one request are not guaranteed to
// exist for a later request, since each may run on a different instance.
// Use in-memory storage here, then upload the buffer straight to Vercel Blob
// (persistent cloud storage) instead of writing to disk.
const uploadSocialMedia = multer({ storage: multer.memoryStorage(), limits: { fileSize: 50 * 1024 * 1024 } }).single("file");
...
router.post("/upload", authMiddleware, (req: any, res) => {
  uploadSocialMedia(req, res, async (err) => {
    if (err) { res.status(400).json({ error: "Upload failed", message: err.message }); return; }
    if (!req.file) { res.status(400).json({ error: "No file provided" }); return; }
    try {
      const filename = `social-${Date.now()}${path.extname(req.file.originalname)}`;
