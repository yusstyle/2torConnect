...
if (-not (Test-Path ".git")) {
    Write-Host "ERROR: Run this from your repo root (where .git lives)." -ForegroundColor Red
    exit 1
}
...
function Insert-AfterLine($path, $anchor, $newLine, $label) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i].Contains($anchor)) { $idx = $i; break }
    }
    if ($idx -eq -1) {
        Write-Host "WARNING: anchor for '$label' not found in $path. Skipping." -ForegroundColor Yellow
        return
    }
    if ($lines[$idx + 1].Contains($newLine.Trim())) {
        Write-Host "SKIP: '$label' already present in $path."
        return
    }
    $lines.Insert($idx + 1, $newLine)
    Save-NoBom $path ((($lines) -join "`r`n") + "`r`n")
    Write-Host "OK: applied '$label' in $path"
}

Write-Host "1) Creating investor Student Requests page..."
$studentRequestsPath = "artifacts/web-app/src/pages/investor/StudentRequests.tsx"
if (Test-Path $studentRequestsPath) {
    Write-Host "SKIP: $studentRequestsPath already exists."
} else {
$pageContent = @'
              ...
              return (
                <motion.div key={post.id}
                  ...
                  <div className="flex items-center gap-3 mt-3 flex-wrap">
                    {post.amountNeeded && (
                      <p className="text-yellow-400 font-bold text-sm">NAIRA_SIGN_PLACEHOLDER{Number(post.amountNeeded).toLocaleString()} needed</p>
                    )}
...
$pageContent = $pageContent.Replace("NAIRA_SIGN_PLACEHOLDER", $naira)
Save-NoBom $studentRequestsPath $pageContent
Write-Host "OK: created $studentRequestsPath"
}

Write-Host "2) Wiring up App.tsx import + route..."
$appPath = "artifacts/web-app/src/App.tsx"
Insert-AfterLine $appPath 'import SponsorUniversitiesPage from "@/pages/investor/Universities";' 'import SponsorStudentRequestsPage from "@/pages/investor/StudentRequests";' "App.tsx import"
Insert-AfterLine $appPath '<Route path="/investor/universities" component={SponsorUniversitiesPage} />' '      <Route path="/investor/student-requests" component={SponsorStudentRequestsPage} />' "App.tsx route"

Write-Host "3) Wiring up DashboardLayout.tsx nav links..."
$navPath = "artifacts/web-app/src/components/layout/DashboardLayout.tsx"
Insert-AfterLine $navPath '{ label: "Dashboard", href: "/investor/dashboard", icon: LayoutDashboard },' '      { label: "Student Requests", href: "/investor/student-requests", icon: HandCoins },' "desktop nav link"
Insert-AfterLine $navPath '{ label: "Home", href: "/investor/dashboard", icon: LayoutDashboard },' '      { label: "Requests", href: "/investor/student-requests", icon: HandCoins },' "mobile nav link"
...
git add artifacts/web-app/src/pages/investor/StudentRequests.tsx artifacts/web-app/src/App.tsx artifacts/web-app/src/components/layout/DashboardLayout.tsx
git commit -m "feat: add investor Student Requests page to browse and message students needing sponsorship"
git push
...
Write-Host ""
Write-Host "Done. Test: log in as investor, check the new 'Student Requests' link in the sidebar, and confirm you can see student sponsorship posts and message them."
