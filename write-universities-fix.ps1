<#
  write-universities-fix.ps1
  --------------------------------------------------------------
  Writes the fixed universities.ts DIRECTLY into your repo -- no
  copy-paste, no separate download to mix up. The full fixed file
  content is embedded below and written straight to the target path.

  Run this from your repo root:
    C:\Users\PC\Desktop\2torconnect2026> .\write-universities-fix.ps1
--------------------------------------------------------------#>

$ErrorActionPreference = "Stop"

$targetRelPath = "artifacts\api-server\src\routes\universities.ts"
$targetPath = Join-Path (Get-Location) $targetRelPath

Write-Host ""
Write-Host "=== Step 1: Confirm target file exists ===" -ForegroundColor Cyan
if (-not (Test-Path $targetPath)) {
    Write-Host "Could not find: $targetPath" -ForegroundColor Red
    Write-Host "Make sure you're running this from C:\Users\PC\Desktop\2torconnect2026" -ForegroundColor Yellow
    exit 1
}
Write-Host "Found: $targetPath" -ForegroundColor Green

Write-Host ""
Write-Host "=== Step 2: Backup current file ===" -ForegroundColor Cyan
$backupPath = "$targetPath.bak"
Copy-Item $targetPath $backupPath -Force
Write-Host "Backed up to: $backupPath" -ForegroundColor Green

Write-Host ""
Write-Host "=== Step 3: Write the fixed file content directly ===" -ForegroundColor Cyan
...
router.get("/search", async (req, res) => {
  ...
  // Local KNOWN_UNIS match first — this is what actually catches acronyms
  ...
  try {
    ...
    // external lookup failed/timed out — local matches (if any) still get returned below
  }
...
Set-Content -Path $targetPath -Value $fixedContent -NoNewline -Encoding UTF8
Write-Host "Wrote fixed content to: $targetPath" -ForegroundColor Green

Write-Host ""
Write-Host "=== Step 4: Verify the fix actually landed ===" -ForegroundColor Cyan
$check = Select-String -Path $targetPath -Pattern "localMatches" -Quiet
if ($check) {
    Write-Host "CONFIRMED: 'localMatches' found in the file -- the fix is in place." -ForegroundColor Green
} else {
    Write-Host "PROBLEM: 'localMatches' NOT found after writing. Something went wrong." -ForegroundColor Red
...
Write-Host ""
Write-Host "=== Step 5: Show git status to prove git sees a real change ===" -ForegroundColor Cyan
git status -- $targetRelPath
git diff --stat -- $targetRelPath

Write-Host ""
Write-Host "=================================================" -ForegroundColor Cyan
Write-Host " Fix is written and verified. Nothing committed yet." -ForegroundColor Green
Write-Host " If 'git diff --stat' above shows changes, you're good to go:" -ForegroundColor Green
Write-Host ""
Write-Host "   git add $targetRelPath" -ForegroundColor White
Write-Host "   git commit -m `"fix: match acronyms like FUD, UNILAG against known universities in search`"" -ForegroundColor White
Write-Host "   git push" -ForegroundColor White
Write-Host "=================================================" -ForegroundColor Cyan

Write-Host ""
Write-Host "=== Also: protect your db-url.txt ===" -ForegroundColor Cyan
if (Test-Path "db-url.txt") {
    $gitignoreContent = if (Test-Path ".gitignore") { Get-Content ".gitignore" -Raw } else { "" }
    if ($gitignoreContent -notmatch "db-url\.txt") {
        Add-Content ".gitignore" "`ndb-url.txt"
        Write-Host "Added db-url.txt to .gitignore so it can never be accidentally committed." -ForegroundColor Green
    } else {
        Write-Host "db-url.txt already in .gitignore." -ForegroundColor Green
    }
}
