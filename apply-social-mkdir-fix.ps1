# apply-social-mkdir-fix.ps1
# Run this from the root of your 2torConnect repo (in PowerShell / pwsh).
#
# Latest error from your logs:
#   Backend failed to start: ENOENT: no such file or directory,
#   mkdir '/var/task/uploads/social'
#
# social.ts ran fs.mkdirSync() at MODULE LOAD TIME (not just when someone
# uploads something), gated only on whether BLOB_READ_WRITE_TOKEN was set.
# Since that env var isn't set in your Vercel production env, it tried to
# create a folder inside /var/task -- Vercel's read-only deployment bundle
# -- which crashed the ENTIRE backend on every request, not just uploads.
#
# Fix: also skip that mkdir whenever process.env.VERCEL is set (matching
# the pattern auth.ts already uses correctly). Uploads without a configured
# Blob token will still cleanly fail with a normal error response at
# request time -- they just won't crash the whole app at boot anymore.
#
# This writes the fixed social.ts, then stages, commits, and pushes it.

$ErrorActionPreference = "Stop"

$targetPath = "artifacts/api-server/src/routes/social.ts"
...
// process.cwd() on Vercel resolves inside /var/task, which is the read-only
// deployment bundle -- mkdirSync there throws and used to crash the whole
// backend at import time, on every single request, regardless of whether
// anyone was even uploading anything. Only attempt the local dev fallback
// dir when we're not running on Vercel at all.
if (!useBlobStorage && !process.env.VERCEL && !fs.existsSync(socialUploadDir)) {
...
git commit -m "fix: never mkdir into read-only Vercel bundle at module load in social.ts (fixes backend crash on every request)"

git push

Write-Host ""
Write-Host "Pushed. Now redeploy the same way that worked last time:"
Write-Host "  - Check Vercel dashboard -> Deployments -> Redeploy from latest main commit, OR"
Write-Host "  - vercel --prod (if the CLI is behaving again)"
Write-Host "Then verify with .\vercel-get-full-logs.ps1 that the deploymentId changed and the ENOENT error is gone."